...
public class UsuarioSpringSecurity implements UserDetails {
	...
	public UsuarioSpringSecurity(Integer id, String email, String senha, Set<Perfil> perfis) {
		...
		this.email = email;
		this.senha = senha;
		this.autoridades = perfis.stream().map(perfil -> new SimpleGrantedAuthority(perfil.getDescricao())).collect(Collectors.toList());
	}
	...
	public boolean temPermicao(Perfil perfil) {
